...
# hydraulic pressure
P_hydrau = (repeat(collect(1:17), 1, 16) * d[3] .+ h) * JutulDarcyRules.ρH2O * 9.807   
...
fig=figure(figsize=(50, 6))
imshow(K', extent=(0, (n[1]-1)*d[1], h+(n[3]-1)*d[3], h), cmap="Blues")
...
plot(inj_loc[1]-d[1], inj_loc[3]+h+d[3]-d[3], "xk", markersize=15)
plot(inj_loc[1]-d[1], inj_loc[3]+h+2*d[3]-d[3], "xk", markersize=15)
plot(inj_loc[1]-d[1], inj_loc[3]+h+3*d[3]-d[3], "xk", markersize=15)
text(inj_loc[1]-0.5*d[1], inj_loc[3]+h, c="k", "Injection", fontsize=20)
...
fig = figure(figsize=(50, 6))
imshow(P_hydrau, extent=(0, (n[1]-1)*d[1], h+(n[3]-1)*d[3], h), cmap="Blues")
im_ratio = size(K, 2)*d[3] / (size(K, 1)*d[1])
clb = colorbar(fraction=0.046*im_ratio, pad=0.04)
...
f = jutulVWell(irate[1], [(inj_loc[1], inj_loc[2])]; startz=[inj_loc[3]], endz=[inj_loc[3]+4*d[3]])
...
fig, axes = subplots(2, 1, figsize=(48, 12))
...
ax1.plot(inj_loc[1]-d[1], inj_loc[3]+h+3*d[3]-d[3], "xk", markersize=15)
ax1.text(inj_loc[1]-0.5*d[1], inj_loc[3]+h, c="k", "Injection", fontsize=20)

fig.colorbarr(im_sat, fraction=0.046*im_ratio, pad=0.04)
...
ax2.plot(inj_loc[1]-d[1], inj_loc[3]+h+3*d[3]-d[3], "xk", markersize=15)
ax2.text(inj_loc[1]-0.5*d[1], inj_loc[3]+h, c="k", "Injection", fontsize=20)
